...
class DatasetTests(unittest.TestCase):
    ...
    def test_cli_task_discovery_and_limit(self):
        ...
        rates = yaml.safe_load((task_file.parent / 'pricing.yaml').read_text())['openai/gpt-5.6-luna']
        cost_file = self.root / 'pricing.yaml'
        cost_file.write_text(yaml.safe_dump({'mockllm/model': rates}))
        ...
        with chdir(task_file.parent), \
            ...
            result = CliRunner().invoke(inspect, [
                ...
                '--log-dir', log_dir,
                '--model-cost-config', str(cost_file),
            ])
            ...
            usage = next(iter(log.stats.model_usage.values()))
            expected = (50 * rates['input'] + usage.output_tokens * rates['output']) / 1_000_000
            self.assertAlmostEqual(usage.total_cost, expected)
            self.assertAlmostEqual(log.samples[0].output.usage.total_cost, expected)


class ScoringTests(unittest.TestCase):
    def test_pricing_counts_cache_and_does_not_double_count_reasoning(self):
        from inspect_ai.model import ModelCost, compute_model_cost
        rates = yaml.safe_load((Path(__file__).resolve().parents[1] / 'pricing.yaml').read_text())
        cost = ModelCost(**rates['openai/gpt-5.6-luna'])
        usage = ModelUsage(input_tokens=3, input_tokens_cache_write=3344,
                           input_tokens_cache_read=0, output_tokens=577,
                           reasoning_tokens=516, total_tokens=3924)
        self.assertAlmostEqual(compute_model_cost(cost, usage), .001529)
        usage.input_tokens_cache_write = 0
        usage.input_tokens_cache_read = 3344
        self.assertAlmostEqual(compute_model_cost(cost, usage), .00075988)
        ...
        def respond(request):
            ...
            if request.url.path.endswith('/responses'):
                ...
                response = {
                    ...
                    'usage': {'prompt_tokens': 3341, 'completion_tokens': 586, 'total_tokens': 3927},
                }
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch.dict(os.environ, {'OPENAI_API_KEY': 'direct-test', 'OPENROUTER_API_KEY': 'router-test'}, clear=True), \
                ...
                for name, key, endpoint in [
                    ...
                    ('openrouter/google/gemini-2.5-pro', 'router-test', 'https://openrouter.ai/api/v1/chat/completions'),
                ]:
                    async with httpx2.AsyncClient(transport=httpx2.MockTransport(respond)) as client:
                        opts = {'responses_api': True} if name.startswith('openai/') else {}
                        model = get_model(name, http_client=client, **opts)
